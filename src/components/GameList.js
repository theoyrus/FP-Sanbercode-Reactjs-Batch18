...
const GameList = () => {
    ...
    const handleEdit = (props) => {
        // console.log(ev)
        let { id } = props
        history.push(`/manage-game/edit/${id}`)
    }

    const handleDelete = (props) => {
        console.log(props)
        let id = props.id
        let gameNama = model.filter(el => el.id == id)[0]?.name
        console.log(gameNama)
        let isHapus = window.confirm(`Apakah yakin akan menghapus game ${gameNama}?`)
        if (isHapus) {
